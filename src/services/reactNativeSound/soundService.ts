  ...
  async playTrack(
    ...
    try {
      ...
      const sound = new Sound(filename, Sound.MAIN_BUNDLE, error => {
        ...
        // Set volume before playing if specified
        if (options?.volume !== undefined) {
          sound.setVolume(Math.max(0, Math.min(1, options.volume)))
          console.log(
            `React Native Sound: Set initial volume to ${options.volume}`,
          )
        }
  ...
  private loadFromRawAssets(
    ...
    const sound = new Sound(filename.replace('.mp3', ''), '', error => {
      ...
      if (volume !== undefined) {
        ...
        console.log(
          `React Native Sound: Set initial volume to ${volume} for raw asset`,
        )
      }
